...
describe('UserService int', () => {
    ...
    describe('signup', () => {
        it('should call repository.create with correct user data and return the result', async () => {
            ...
            expect(mockUserRepository.create.calledWithMatch({
        ...
        it('should throw an error if repository.create throws', async () => {
            mockUserRepository.create.rejects(new Error('Error when creating user in database.'));
            await expect(userService.signup(mockSignupRequest)).rejects.toThrow('Error when creating user in database.');
        });
    });
});